...
import { Calendar, Plus, Loader2, Download, CheckCircle, Clock, Search, ArrowUpDown, Filter, User, ShieldAlert } from 'lucide-react';
...
export default function AttendanceAdmin() {
  ...
  const fetchPrograms = async () => {
    try {
      setLoading(true);
      setError(null);
      const { data, error: prgErr } = await supabase
        .from('programs')
        .select('*')
      ...
      console.error(err); 
      setError("Programs: " + (err.message || "Failed to fetch programs"));
    }
  ...
  const fetchRecords = async () => {
    try {
      ...
      const { data, error: recErr } = await supabase
        ...
        .select('*')
        .order('event_name', { ascending: sortOrder === 'asc' });
      
      if (recErr) throw recErr;
      if (data) setRecords(data);
    } catch (err: any) {
      console.error(err);
      setError("Records: " + (err.message || "Failed to fetch records"));
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => { 
    if (activeView === 'programs') fetchPrograms();
    else fetchRecords();
  ...
  const exportData = () => {
    ...
    link.setAttribute("download", `nss_attendance_${activeView}_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button 
            onClick={() => setActiveView(activeView === 'programs' ? 'records' : 'programs')}
          ...
          >
            {activeView === 'programs' ? 'View All Attendance' : 'Manage Programs'}
          </button>
          <button 
            onClick={exportData}
      ...
      {activeView === 'programs' ? (
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 min-h-[400px]">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <h3 className="font-bold text-slate-900 uppercase tracking-widest text-[10px]">Attendance Master Records</h3>
            <div className="flex items-center gap-3">
              <div className="relative">
                <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input 
                  type="text" 
                  placeholder="Search name, unit or program..." 
                  value={searchTerm}
              ...
              <button 
                ...
                className="h-10 px-3 flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl text-[10px] font-black uppercase tracking-wider text-slate-600 hover:bg-slate-100"
              >
                <ArrowUpDown size={12} />
                Program: {sortOrder === 'asc' ? 'A-Z' : 'Z-A'}
              </button>
            </div>
          </div>
            ...
            <table className="w-full border-collapse">
              ...
              <tbody className="divide-y divide-slate-50">
                {loading ? (
                  ...
                  <tr><td colSpan={4} className="py-20 text-center text-slate-400 font-medium italic">No records found.</td></tr>
                  ...
                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4">
                      <span className="px-2 py-1 bg-brand-50 text-brand-600 text-[10px] font-black uppercase rounded">
                        {record.event_name}
                      </span>
                    </td>
